...
class RoomsDAO(BaseDAO):
    ...
    async def get_rooms_info_of_hotel(
        ...
        async with async_session_maker() as session:
            ...
            booked_rooms = select(booked_rooms_with_hotel_id.c.room_id, func.count(booked_rooms_with_hotel_id.c.id).label("booked_amount")).where(
                and_(
                    or_(
                        and_(
            ...
            ).group_by(booked_rooms_with_hotel_id.c.room_id).cte("booked_rooms")

            rooms_with_needed_info = select(Rooms, (Rooms.price * days_total).label("total_cost"),
                                            (Rooms.quantity - func.coalesce(booked_rooms.c.booked_amount, 0)).label("rooms_left")).outerjoin(
