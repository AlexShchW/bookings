...
class HotelsDAO(BaseDAO):
    ...
    @classmethod
    async def get_available_hotels(cls, location: str, date_from: date, date_to: date):
        async with async_session_maker() as session:
            ...
            hotels_ids_with_rooms_occupied = (
                select(
                    ...
                    func.count(booked_rooms_with_hotel_id.c.id).label("rooms_booked"),
                )
                .group_by(booked_rooms_with_hotel_id.c.hotel_id)
            ...
            hotels_with_vacant_rooms = (
                select(
                    ...
                    (
                        ...
                        - func.coalesce(
                            hotels_ids_with_rooms_occupied.c.rooms_booked, 0
                        )
                    ).label("rooms_left"),
                ).outerjoin(
                    hotels_ids_with_rooms_occupied,
            ...
            res = select(hotels_with_vacant_rooms).where(
                and_(
